...
find_python() {
  ...
  for c in "${EIGHTYEIGHT_PYTHON:-}" "${PLUGIN_PYTHON:-}" "${RFFP_PYTHON:-}"; do
    _try "$c" && return 0
  done
...
PY="$(find_python)" || {
  echo "recover-from-false-positive: no Python >=${MIN_MAJOR}.${MIN_MINOR} found." >&2
  echo "  Set EIGHTYEIGHT_PYTHON=/path/to/python3 (or RFFP_PYTHON), or install Python 3." >&2
